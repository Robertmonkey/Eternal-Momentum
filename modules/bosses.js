...
import * as utils from './utils.js'; // <-- FIX: Added missing import for utils
...
export const bossData = [{
    ...
    logic: (b, ctx, state, utils, gameHelpers) => {
            ...
            if (availablePools.length > 0) {
                ...
                if (aspectData) {
                    ...
                    b.activeAspects.set(aspectId, {
                        id: aspectId,
                        type: poolToUse,
                        endTime: now + (poolToUse === 'primary' ? 16000 : 15000), // Increased duration for primary
                    });
        ...
        // --- COLLISION WITH SPAWNED PILLARS ---
        if (b.pillars) { // Architect pillars
        ...
        if (b.pillar) { // Annihilator pillar
        ...
        // --- BASE MOVEMENT ---
        if (!b.activeAspects.has('juggernaut')) { // Juggernaut takes full control of movement
             b.dx = (state.player.x - b.x) * 0.001; // Tuned speed
             b.dy = (state.player.y - b.y) * 0.001; // Tuned speed
             b.x += b.dx;
             b.y += b.dy;
        }
